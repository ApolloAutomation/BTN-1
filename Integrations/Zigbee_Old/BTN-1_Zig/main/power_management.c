...
esp_err_t power_management_init(const power_config_t *config)
{
    ...
    /* ESP32-C6 with Zigbee doesn't support dynamic frequency scaling the same way */
    /* The Zigbee stack will handle sleep modes internally */
    /* Skip PM configuration that's not supported on ESP32-C6 */
    ESP_LOGI(TAG, "ESP32-C6 Zigbee mode - power management handled by Zigbee stack");
    
    /* No need for PM locks on ESP32-C6 in Zigbee mode */
    g_pm_state.pm_lock_cpu_freq_max = NULL;
    ...
    ESP_LOGI(TAG, "Power management initialized (Zigbee sleep mode enabled)");
...
static esp_err_t configure_gpio_wakeup(void)
{
    ...
    /* For ESP32-C6, use ext1 wakeup with ANY_LOW instead of deprecated ALL_LOW */
...
esp_err_t power_acquire_wake_lock(wake_lock_type_t type, uint32_t timeout_ms)
{
    ...
    if (xSemaphoreTake(g_pm_state.wake_locks[type], 0) == pdFALSE) {
        ...
        /* No PM lock needed for ESP32-C6 in Zigbee mode */
        if (g_pm_state.pm_lock_cpu_freq_max) {
...
esp_err_t power_release_wake_lock(wake_lock_type_t type)
{
    ...
    if (xSemaphoreGive(g_pm_state.wake_locks[type]) == pdTRUE) {
        ...
        if (!power_has_wake_locks()) {
            /* Release CPU frequency lock if it exists */
            if (g_pm_state.pm_lock_cpu_freq_max) {
