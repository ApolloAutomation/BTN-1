...
void btn1_button_event_handler(button_event_t *event)
{
    ...
    ESP_LOGI(TAG, "Button %d event: %s", event->button_num,
             ...
             event->event == BUTTON_EVENT_DOUBLE_PRESS ? "DOUBLE_PRESS" :
             event->event == BUTTON_EVENT_PAIRING_MODE ? "PAIRING_MODE" :
             event->event == BUTTON_EVENT_FACTORY_RESET ? "FACTORY_RESET" : "UNKNOWN");
    
    /* Handle special events first */
    switch (event->event) {
    case BUTTON_EVENT_PAIRING_MODE:
        /* Enter pairing mode */
        ESP_LOGI(TAG, "Entering pairing mode via button press");
        zigbee_enter_pairing_mode();
        return;
        
    case BUTTON_EVENT_FACTORY_RESET:
        /* Perform factory reset */
        ESP_LOGI(TAG, "Factory reset triggered via button combination");
    ...
    /* Ensure button number is valid for normal events */
...
/* Enter pairing mode - open network for joining */
esp_err_t zigbee_enter_pairing_mode(void)
{
    ESP_LOGI(TAG, "Entering pairing mode - opening network for 180 seconds");
    
    /* Show LED pairing mode indication */
    simple_led_set_pattern(LED_PATTERN_PAIRING);
    
    /* Check if we're already in a network */
    if (!g_network_joined) {
        /* Open the network for joining (180 seconds) */
        esp_err_t err = esp_zb_bdb_open_network(180);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Failed to open network for joining: %s", esp_err_to_name(err));
            /* Show error indication */
            simple_led_set_pattern(LED_PATTERN_ERROR);
            return err;
        }
        
        ESP_LOGI(TAG, "Network opened for joining - 180 second window");
        
        /* Start network steering to join a network */
        ESP_LOGI(TAG, "Starting network steering for pairing");
        /* Show joining indication */
        simple_led_set_pattern(LED_PATTERN_JOINING);
        err = esp_zb_bdb_start_top_level_commissioning(ESP_ZB_BDB_NETWORK_STEERING);
        if (err != ESP_OK) {
        ...
        ESP_LOGI(TAG, "Already joined to a network - leave first to rejoin");
        /* Show error indication */
        simple_led_set_pattern(LED_PATTERN_ERROR);
        return ESP_ERR_INVALID_STATE;
    }
    
    /* Acquire wake lock to keep device awake during pairing */
    power_acquire_wake_lock(WAKE_LOCK_ZIGBEE, 180000);  /* 3 minutes */
    
    ESP_LOGI(TAG, "Pairing mode activated - press permit join in Home Assistant");
    ESP_LOGI(TAG, "Device will attempt to join available networks for 180 seconds");
    
    return ESP_OK;
}

/* Leave the current Zigbee network */
esp_err_t zigbee_leave_network(void)
{
    ESP_LOGI(TAG, "Leaving current Zigbee network");
    
    /* Check if we're in a network */
    if (g_network_joined) {
        /* Use the Zigbee BDB (Base Device Behavior) leave method */
        /* This will properly leave the network and clear network data */
        esp_err_t err = esp_zb_bdb_start_top_level_commissioning(ESP_ZB_BDB_MODE_INITIALIZATION);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Failed to leave network: %s", esp_err_to_name(err));
            return err;
        }
        
        g_network_joined = false;
        ESP_LOGI(TAG, "Network leave initiated");
        
        /* Clear network data to ensure clean state */
        esp_zb_nvram_erase_at_start(true);
    } else {
        ESP_LOGI(TAG, "Not currently in a network");
    }
...
esp_err_t zigbee_factory_reset(void)
{
    ...
    simple_led_set_pattern(LED_PATTERN_FACTORY_RESET);
    
    /* Clear all bindings and settings */
    btn1_clear_all_bindings();
    ...
    /* Clear Zigbee network data */
    ESP_LOGI(TAG, "Clearing Zigbee network data");
    esp_zb_factory_reset();
    
    /* Log factory reset completion */
    ESP_LOGI(TAG, "Factory reset complete - restarting device in 3 seconds");
    
    /* Delay before restart to allow logs to be sent and LED pattern to complete */
...
/* Update network joined state - to be called from signal handler */
void zigbee_set_network_joined(bool joined)
{
    g_network_joined = joined;
    ESP_LOGI(TAG, "Network joined state: %s", joined ? "JOINED" : "NOT JOINED");
    
    /* Show appropriate LED indication */
    if (joined) {
        simple_led_set_pattern(LED_PATTERN_CONNECTED);
    } else {
        /* If not joined and not in an error state, stop LED indication */
        // Temporarily disabled - LED implementation needs update
        // if (led_indicator_get_pattern() != LED_PATTERN_ERROR) {
        //     led_indicator_stop();
        // }
    }
}