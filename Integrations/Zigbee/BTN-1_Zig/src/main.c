...
void esp_zb_app_signal_handler(esp_zb_app_signal_t *signal)
{
    ...
    case ESP_ZB_BDB_SIGNAL_STEERING:
        if (err_status == ESP_OK) {
            ...
            esp_zb_scheduler_alarm((esp_zb_callback_t)esp_zb_bdb_start_top_level_commissioning, ESP_ZB_BDB_NETWORK_STEERING, 1000);
        }
        break;
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(power_management_init(&pm_config));
